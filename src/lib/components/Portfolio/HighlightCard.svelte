<script lang="ts">
	import type { ProjectsProps } from '@components/Portfolio/projects';

	type Props = ProjectsProps & {
		i: number;
	};

	const { title, subtitle, stats, featuredImage, link, i }: Props = $props();
</script>
...
>
	<div
		class="border-gradient relative top-0 mx-auto flex items-center justify-center rounded-lg py-10 px-12"
	>
		<div
		...
		>
			<div class="md:row-span-2">
				<div class="flex h-full flex-col justify-between">
					<div>
						...
						<p class="text-offwhite mt-4 text-base md:text-lg">
							{subtitle}
						</p>
					</div>
					{#if link}
			...
			<div class="row-start-1 md:col-start-2">
				<div class="relative aspect-video w-full rounded-md bg-gray-400"></div>
			</div>
			<div class="md:col-start-2">
				<div class="flex flex-col gap-4">
					<div class="project-stats flex flex-col justify-around gap-4 lg:flex-row">
						{#each stats as stat}
							<div class="stat flex flex-col flex-1">
...
<style>
	.border-gradient {
		...
		&::before {
			...
			background: linear-gradient(135deg, var(--color-blue) 20%, var(--color-orange-light));
		}
		&::after {
			content: '';
			position: absolute;
